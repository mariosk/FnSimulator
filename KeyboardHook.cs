...
namespace gma.System.Windows
{
    ...
    public class KeyboardHook
    {
        ...
        private int[] numbers;
        private int[] capNumbers;
        private int[] characters;
        private int[] fnCharacters;
        ...
        public void Start(bool InstallMouseHook, bool InstallKeyboardHook)
        {
            if (numbers == null)
            {
                ...
                capNumbers[5] = 73; // 5
                capNumbers[6] = 79; // 6
                characters = new int[5];
                characters[0] = 48; // '/' -> 47
                characters[1] = 112; // '*' -> 42
                characters[2] = 80; // '*' -> 42 (with CAPS)
                characters[3] = 59; // '-' -> 45
                characters[4] = 47; // '+' -> 43
                fnCharacters = new int[5];
                fnCharacters[0] = 47; // '/' -> 47
                fnCharacters[1] = 42; // '*' -> 42
                fnCharacters[2] = 42; // '*' -> 42 (with CAPS)
                fnCharacters[3] = 45; // '-' -> 45
                fnCharacters[4] = 43; // '+' -> 43
            }
        ...
        private void PressKeyboardButton(Keys keyCode, KeyPressEventArgs e)
        {
            ...
            //Console.WriteLine(keyCode + " " + e.KeyChar);
            if (e != null) e.Handled = true;
            calledArtificially = true;
            keybd_event((byte)keyCode, 0, KEYEVENTF_KEYDOWN, 0); // press key
            keybd_event((byte)keyCode, 0, KEYEVENTF_KEYUP, 0); // release key            
            calledArtificially = false;
            //keybd_event((byte)keyCode, 0x45, KEYEVENTF_EXTENDEDKEY, 0);
            //keybd_event((byte)keyCode, 0x45, KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYDOWN, 0);
        }

        private int FindNumber(int[] array, int keyChar)
        {
        ...
        private int FindNumber(int[] array, int[] retArray, int keyChar)
        {
            if (array.Length != retArray.Length)
                return -1;
